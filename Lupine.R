...
saveRDS(physeq, "phyloseq_roots.rds")
...
###########################################################################################
#### Correlation network using MixOmics and rCCA for time point 3 (2nd harvest)###
###########################################################################################
...
# create your axis
X <- read.csv('Met_NSST_lupine_t3.csv',dec='.')
Y <-read.csv('Abund_NSST_lupine_t3.csv',dec='.')

# create the grid to plot
grid1 <- seq(0.001, 0.2, length = 51)
grid2 <- seq(0.001, 0.2, length = 51)

# estimate the parameters for regularizing the Canonical Correlation Analysis (CCA) 
cv.score<-tune.rcc(X,Y, grid1=grid1,grid2=grid2, validation = "loo")

# this is the code that worked with more links and changing the threshold
tpt3.res <- rcc(X,Y,ncomp=3,lambda1 = 0.064,lambda2 = 0.008)

#this is the code for plotting the network


pdf(p_NSNT_lupine_t3, file = "rCCA_NS_ST_lupine_t3.pdf", width = 12, height = 9)
p_NSNT_lupine_t3 <- network(tpt3.res, comp = 1:3,cutoff = 0.8,
                            color.node = c("azure", "white"),
                            shape.node = c("circle", "rectangle"),
                            color.edge = color.jet(8),
...
png(p_NSNT_lupine_t3, file = "rCCA_NS_ST_lupine_t3.png", width = 12, height = 9, units = "in", res = 500)
p_NSNT_lupine_t3 <- network(tpt3.res, comp = 1:3,cutoff = 0.8,
                            color.node = c("azure", "white"),
                            shape.node = c("circle", "rectangle"),
                            color.edge = color.jet(8),
...
X <- read.csv('Met_NSOR_lupine_t3.csv',dec='.')
Y <-read.csv('Abund_NSOR_lupine_t3.csv',dec='.')
...
pdf(p_NSNT_lupine_t3, file = "rCCA_NS_OR_lupine_t3.pdf", width = 12, height = 9)
p_NSOR_lupine_t3 <- network(tpt3.res, comp = 1:3,cutoff = 0.82,
                            color.node = c("azure", "white"),
                            shape.node = c("circle", "rectangle"),
                            color.edge = color.jet(8),
                            lty.edge = "solid", lwd.edge = 2, 
...
X <- read.csv('Met_NSNF_lupine_t3.csv',dec='.')
Y <-read.csv('Abund_NSNF_lupine_t3.csv',dec='.')
...
pdf(p_NSNT_lupine_t3, file = "rCCA_NS_NF_lupine_t3.pdf", width = 12, height = 9)
p_NSNF_lupine_t3 <- network(tpt3.res, comp = 1:3,cutoff = 0.78,
                            color.node = c("azure", "white"),
                            shape.node = c("circle", "rectangle"),
                            color.edge = color.jet(8),
                            lty.edge = "solid", lwd.edge = 2, 
...
#########################################################################################
####### DESEq Analysis to find differences in relative abundances between fertilizers ###
#########################################################################################
# load library
library("DESeq2")
library("phyloseq")
...
# import data
# physeq <- readRDS("./phyloseq_roots.rds")

# check if all is there
sample_data(physeq)

# Test for fertilizer but controlling for timepoint
diagdds = phyloseq_to_deseq2(physeq, ~ Timepoint + Fertilizer)

# Run DESeq
dds <- DESeq2::DESeq(diagdds, quiet = TRUE, test = "Wald", fitType = "parametric")

# Calculate contrasts for all comparisons
comp1 <- DESeq2::results(dds, contrast=c("Fertilizer","No_fertilizer","Organic"))[order(results(dds)$padj), ]
comp2 <- DESeq2::results(dds, contrast=c("Fertilizer","No_fertilizer","Struvite"))[order(results(dds)$padj), ]
comp3 <- DESeq2::results(dds, contrast=c("Fertilizer","Organic","Struvite"))[order(results(dds)$padj), ]

overall_deseq_results <- data.frame(OTU = c(rownames(comp1), rownames(comp2), rownames(comp3)),
                                    rbind(comp1, comp2, comp3),
                                    comparison =
                                      c(rep("No_fertilizer-Organic", nrow(comp1)),
                                        rep("No_fertilizer-Struvite", nrow(comp2)),
                                        rep("Organic-Struvite", nrow(comp3))),
                                    design = "~ Timepoint + Fertilizer"
)


# Filter significant results
overall_deseq_results <- overall_deseq_results %>%
  dplyr::filter(padj < 0.05)

# get taxonmy
tax_sb <- data.frame(tax_table(prune_taxa(as.character(overall_deseq_results$OTU), physeq)))
tax_sb <- data.frame(OTU = rownames(tax_sb),
                     tax_sb)

# add taxonomy
overall_deseq_results <- overall_deseq_results %>%
  left_join(., tax_sb, by = "OTU")


# Focus on rhizobium
physeq_rel_abund <- transform_sample_counts(physeq, function(x) x/sum(x))
df <- psmelt(physeq_rel_abund)
df <- df%>%
  dplyr::filter(Genus == "Rhizobium") %>%
  distinct()

df$Fertilizer <- gsub("No_fertilizer", "No fertilizer", df$Fertilizer)

p1_rhizo <- df %>%
  ggplot(., aes(x = Fertilizer, y = 100*Abundance, fill = OTU))+
  # geom_point(shape = 21, size = 4)+
  geom_boxplot()+
  theme_bw()+
  scale_y_log10()+
  facet_grid(Timepoint~.)+
  scale_fill_brewer("Rhizobium\nOTUs", palette = "Accent")+
  theme(axis.text=element_text(size=12), axis.title=element_text(size=14),
        axis.text.x = element_text(size = 12),
        title=element_text(size=13), legend.text=element_text(size=12),
...
png("p1_rhizo.png", res = 500, width = 5, height = 6, units = "in")
p1_rhizo
dev.off()

overall_deseq_results$comparison <- gsub("No_fertilizer", "No fertilizer", overall_deseq_results$comparison)

p1_logfc <- overall_deseq_results %>%
  ggplot(., aes(x = OTU, y = log2FoldChange, fill = Genus))+
  # geom_point(shape = 21, size = 4)+
  # geom_boxplot()+
  ...
  scale_fill_brewer("Genus", palette = "Accent")+
  theme(axis.text=element_text(size=12), axis.title=element_text(size=14),
        axis.text.x = element_text(size = 8),
        title=element_text(size=13), legend.text=element_text(size=12),
        legend.background = element_rect(fill="transparent"),
        strip.text=element_text(size=8), legend.position = "right",
        strip.background = element_rect(fill = adjustcolor("gray", 0.15)))+
  labs(x = "", y = "log2foldchange")+
  geom_hline(yintercept = 0)+
  geom_errorbar(aes(ymax = log2FoldChange + lfcSE, ymin = log2FoldChange - lfcSE), width = 0.05)

png("p1_logfc.png", res = 500, width = 9, height = 4, units = "in")
p1_logfc
dev.off()